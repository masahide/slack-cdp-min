...
// === ★ 追加：最終フォールバックで Slack ページ内から元メッセージを取得 ===
async function fetchMessageViaPage(opts: {
  Runtime: RuntimeLike; // ← any をやめる
  channel: string;
  ts: string;
}): Promise<{ text?: string; user?: string } | null> {
  const { Runtime, channel, ts } = opts;

  // Slack ページ内で conversations.replies を叩く。token/cookie はページに任せる。
  const expression = `
  (async () => {
    try {
      const origin = location.origin;
      const body = new URLSearchParams({ channel: ${JSON.stringify(channel)}, ts: ${JSON.stringify(ts)}, limit: "1", inclusive: "true" });
      // token は Fetch の Authorization ヘッダに自動で入らない場合があるので、取得できれば付与
      const token =
        (window as any)?.TS?.boot_data?.api_token ||
        (window as any)?.boot_data?.api_token || null;

      const headers: Record<string,string> = { "Content-Type": "application/x-www-form-urlencoded" };
      if (token) headers["Authorization"] = "Bearer " + token;

      const resp = await fetch(origin + "/api/conversations.replies", {
        method: "POST",
        headers,
        body,
        credentials: "include",
      ...
      // blocks があれば text は省略されることがあるので両方返す
      return { text: m.text ?? null, blocks: m.blocks ?? null, user: m.user ?? m.user_id ?? null };
    } catch (e) {
      return null;
    }
  })()
  ...
  if (!val) return null;
  const text = val.text ?? fromBlocks(val.blocks);
  const user = val.user ?? undefined;
  return { text, user };
}

// === ★ 追加：リアクション時に「確実に」元投稿を取っておくユーティリティ ===
async function ensureOriginalMessage(opts: {
  Runtime: RuntimeLike; // ← any をやめる
  channel: string;
  ts: string;
}): Promise<{ text?: string; user?: string } | null> {
  const { Runtime, channel, ts } = opts;
  const k = key(channel, ts);

  // ① 既にキャッシュにあればそれを使う
  const c = cache.get(k);
  if (c?.text || c?.user) return c;

  // ② 取れなければ Slack ページ内から直接取得（最終手段）
  const fetched = await fetchMessageViaPage({ Runtime, channel, ts });
  if (fetched) {
    cache.set(k, fetched);
    return fetched;
  }
...
async function main() {
  ...
  const { Network, Fetch, Runtime } = client;
      ...
      if (mm) {
        ...
        if (isPost) {
          ...
          if (c?.text || c?.user) {
            ...
            const fetched = await ensureOriginalMessage({ Runtime, channel, ts });
            if (fetched)
              console.log("  ↳ message fetched:", { text: fetched.text, user: fetched.user });
          }
        }
